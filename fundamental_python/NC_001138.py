...
for i in dat["features"]:
    if i["feature"] == "CDS":
        ...
        if i["position"].startswith("complement"):
            fasta = ''.join(''.join(complement.get(base) for base in reversed(dat["sequence"][int(l[0])-1:int(l[1])])) for l in m)
        else:
            fasta = ''.join((dat["sequence"][int(l[0])-1:int(l[1])]) for l in m)
